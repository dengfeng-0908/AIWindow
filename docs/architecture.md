...
- URL、搜索结果和通用网页标题不会作为帖子标题保存或展示；旧异常值先显示为带主题编号的占位。用户在收藏列表点击编号项时会直接打开该主题；只有页面声明的 canonical URL 与当前主题一致时，App 才从已渲染页面更新真实标题，不会后台批量刷新历史记录。
...
- 设置页提供 Kimi、DeepSeek、GLM 和 OpenAI GPT-5.6 预设，自动选择各自的 Chat Completions HTTPS 地址，并允许选择该服务支持的模型与推理强度；自定义兼容服务继续由用户填写完整 HTTPS 地址和模型名，默认不附加厂商特有推理字段。
- 服务商、模型、推理强度和自定义地址进入普通本地设置，密钥只存入仅当前设备、解锁后可用的 Keychain。旧版本已经保存的地址与模型会按自定义服务读取，不会被预设覆盖。
...
- 请求按服务商分别映射 `reasoning_effort` 与 `thinking`，不会把某家的推理字段无差别发送给其他服务；请求仍从 iPhone 直接到用户选择的模型服务。AIWindow 不提供中转后端，不接收 API Key、帖子内容或模型结果。
- 官方预设按各自协议限制最多生成 16,384 tokens，并为较高推理强度提供有限延长的请求等待时间；自定义服务为兼容性不自动添加这些字段。
